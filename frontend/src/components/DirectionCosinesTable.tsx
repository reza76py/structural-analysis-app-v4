...
const DirectionCosinesTable = () => {
  ...
  useEffect(() => {
    const fetchDirectionCosines = async () => {
      try {
        const response = await axios.get("http://127.0.0.1:8000/api/elements/direction-cosines/");
  ...
  return (
    <div className="mt-6">
      <h2 className="text-lg font-bold mb-2">Direction Cosines per Element</h2>
      <table className="table-auto w-full border-collapse border border-gray-300 text-sm">
        ...
        <tbody>
          {data.map(({ id, startNode, endNode, cos_x, cos_y, cos_z, length }) => (
            <tr key={id}>
              <td className="border px-2 py-1 text-center">{id}</td>
              <td className="border px-2 py-1 text-center">{startNode}</td>
              <td className="border px-2 py-1 text-center">{endNode}</td>
              <td className="border px-2 py-1 text-center">{cos_x}</td>
              <td className="border px-2 py-1 text-center">{cos_y}</td>
              <td className="border px-2 py-1 text-center">{cos_z}</td>
              <td className="border px-2 py-1 text-center">{length}</td>
            </tr>
          ))}
        </tbody>
      </table>
