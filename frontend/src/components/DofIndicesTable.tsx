...
// ✅ Added for typing load_vector entries
type LoadEntry = {
    dof: number;
    P_value: number;
};
        ...
        const fetchData = async () => {
            try {
                const [dofRes, loadRes] = await Promise.all([
                    axios.get("${import.meta.env.VITE_API_BASE_URL}/api/dof-indices/"),
                    axios.get("${import.meta.env.VITE_API_BASE_URL}/api/load-vector/"),
                ]);
                ...
                loadRes.data.load_vector.forEach((entry: LoadEntry) => {
                ...
                const dofRows = Object.values(dofMap).sort(
                    (a, b) => a.dof - b.dof,
                );
                setRows(dofRows);
            } catch (error) {
