...
const SolveDisplacement: FC = () => {
  const [displacements, setDisplacements] = useState<Displacement[] | null>(null);
  ...
  const fetchDisplacement = async () => {
    ...
    setError("");
    try {
      const response = await axios.get("http://127.0.0.1:8000/api/solve-displacement/");
  ...
  return (
    <div className="displacement-container">
      ...
      <button onClick={fetchDisplacement} disabled={loading} className="solve-btn">
