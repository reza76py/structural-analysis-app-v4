  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const dofRes = await axios.get("http://127.0.0.1:8000/api/dof-indices/");
        const elementDOFs: ElementDOF[] = dofRes.data.dof_indices;

        // 🟢 Get all nodes to determine ordering
        const nodesRes = await axios.get("http://127.0.0.1:8000/api/nodes/");
        ...
        <tbody>
          {data.map(({ id, startNode, endNode, dof_indices }) => (
            <tr key={id}>
              <td>{id}</td>
              <td>
                {getNodeNumber(startNode)} → {getNodeNumber(endNode)}
              </td>
              <td>[{dof_indices.map((dof) => dof + 1).join(", ")}]</td>
            </tr>
          ))}
        </tbody>
      </table>
