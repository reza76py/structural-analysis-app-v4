...
const ElementGlobalStiffnessMatrices = () => {
  ...
  useEffect(() => {
    const fetchMatrices = async () => {
      try {
        ...
        setElementMatrices(response.data.element_global_stiffness_matrices);
      } catch (error) {
        console.error("Error fetching element-wise global stiffness matrices:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="matrix-display">
      <h2 className="form-title">Element-wise Global Stiffness Matrices</h2>
      {loading ? (
        <p>Loading...</p>
      ) : (
        elementMatrices.map((el) => (
          <div key={el.id} className="matrix-block">
            <h4>Element {el.id}: ({el.startNode} → {el.endNode})</h4>
